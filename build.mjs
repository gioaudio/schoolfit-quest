...
/* Guard 1 — unresolved git merge conflicts. These are valid text but
   invalid JavaScript, so they kill the whole script silently: the page
   renders, and nothing works. Easy to commit by accident. */
const CONFLICT = /^(<{7}|={7}|>{7})/m;
for (const f of [...JS_FILES, "src/styles.css", "src/body.html"]) {
  if (CONFLICT.test(read(f))) {
    console.error(
      "Build aborted — unresolved merge conflict in " + f + "\n" +
      "  Search that file for <<<<<<< and remove the conflict markers,\n" +
      "  keeping whichever version you actually want."
    );
    process.exit(1);
  }
}

/* Guard 2 — fail loudly rather than shipping a broken page. */
try {
